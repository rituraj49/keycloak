...
public class EmailAuthenticatorForm extends AbstractUsernameFormAuthenticator {
    ...
    private void generateAndSendEmailCode(AuthenticationFlowContext context) {
        ...
        sendSmsWithCode(context.getSession(), context.getRealm(), context.getUser(), code, ttl);
    ...
    private void sendSmsWithCode(KeycloakSession session, RealmModel realm, UserModel user, String code, int ttl) {
        if ( user.getFirstAttribute("phoneNumber") == null) {
            log.warnf("Could not send access code phone due to missing phone number. realm=%s user=%s", realm.getId(), user.getUsername());
            throw new AuthenticationFlowException(AuthenticationFlowError.INVALID_USER);
        }

        String phoneNumber = user.getFirstAttribute("phoneNumber");
        String message = "Your otp for logging in is: " + code;

        String realmName = realm.getDisplayName() != null ? realm.getDisplayName() : realm.getName();
        List<Object> subjectParams = List.of(realmName);
        try {
              SmsService.sendSmsOtp(phoneNumber, message);
        } catch (Exception eex) {
            log.errorf(eex, "Failed to send access code otp. realm=%s user=%s", realm.getId(), user.getUsername());
        }
    }
}
